...
- **In dev mode a `.env` file beats shell env vars.** Expo 54's dev server
  bundles `.env` into the browser (`expo/virtual/env`) and spreads it over
  `process.env`; `EXPO_NO_DOTENV` doesn't stop that. So `verify.sh` runs both
  smoke servers with `EXPO_NO_DOTENV=1 ... --no-dev --clear` (values inlined
  from the shell), and `EXPO_PUBLIC_DEMO_MODE=1` forces demo mode. Live and
  demo need separate servers (:8081 demo, :8082 live).
- **iOS builds:** `eas.json` (production profile) carries the public Supabase
  URL/key; bundle ID is `com.megangroothuis.pulse`. `ios/` is generated by
  `npx expo prebuild -p ios --clean --no-install` from `app.config.js`, so
  change the config and regenerate rather than editing `ios/` by hand. The
  Supabase Apple provider's client IDs must include the bundle ID.
